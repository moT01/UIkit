...
| `format`                 | `oxfmt --write . && prettier --write "**/*.{astro,md,mdx,yaml,yml}"`                           | Format js/ts/json with oxfmt; astro/md/mdx/yaml with prettier.                           |
| `format:check`           | `oxfmt --check . && prettier --check "**/*.{astro,md,mdx,yaml,yml}"`                           | Check formatting without writes.                                                         |
| `lint:fix`               | `oxlint --fix`                                                                                 | Apply oxlint auto-fixable findings.                                                      |
| `check:node-versions`    | `node scripts/check-node-versions.mjs`                                                         | Assert Node + `@types/node` floor consistency across the repo.                           |
