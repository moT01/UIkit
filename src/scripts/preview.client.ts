const qs = <T extends Element = Element>(root: ParentNode, sel: string): T | null =>
...
function wireTabs(): void {
  qsa<HTMLElement>(document, '[data-tabs]').forEach((group) => {
    ...
    const activate = (key: string): void => {
      tabs.forEach((t) => t.setAttribute('aria-selected', t.dataset.tabsTab === key ? 'true' : 'false'));
      panels.forEach((p) => (p.hidden = p.dataset.tabsPanel !== key));
    };
    tabs.forEach((t) => t.addEventListener('click', () => activate(t.dataset.tabsTab ?? '')));
  });
}
